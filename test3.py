async def check_last_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute('''
        SELECT p.yookassa_payment_id, p.status, p.amount, p.created_at,
               c.name as company_name, ca.actual_start_date
        FROM payments p
        LEFT JOIN company_arcs ca ON p.company_arc_id = ca.company_arc_id
    ...
    if payment:
        payment_id, status, amount, created_at, company_name, start_date = payment
        
        message = f"💰 **Информация о последнем платеже**\n\n"
        ...
        if company_name:
            ...
            if start_date:
                message += f"**Старт тренинга:** {start_date}\n"
        
        # Создаем кнопку для проверки этого платежа
        keyboard = [[InlineKeyboardButton("🔍 Проверить этот платеж", callback_data=f"check_payment_{payment_id}")]]
        ...
        await update.message.reply_text(
            "❌ **У вас нет платежей в истории.**\n\n"
            "Сначала совершите оплату через каталог тренинга.",
            parse_mode='Markdown'
        )

# Добавляем в main():
application.add_handler(CommandHandler("lastpay", check_last_payment))
