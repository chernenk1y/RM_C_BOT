...
def split_message(text, max_length=4096):
    """Разбивает длинное сообщение на части по max_length символов с учетом Markdown"""
    ...
    while text:
        ...
        for delimiter in ['\n\n', '\n', '. ', '! ', '? ', ' ']:
            pos = text.rfind(delimiter, 0, max_length - 100)
            if pos > 0:
    ...
    for i, part in enumerate(parts):
        if len(part) > max_length:
            print(f"⚠️ Часть {i} всё ещё слишком длинная: {len(part)} символов")
            parts[i:i+1] = [part[j:j+max_length] for j in range(0, len(part), max_length)]
...
TOKEN = "8168840313:AAE7Ju-uWkbUkEExz2jgt9xcbFuLgpQEerA"
init_db()
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    unique_buttons = {
        ...
        "🛠 Тех.поддержка": lambda u, c: u.message.reply_text("Помощь по боту (в разработке)"),
...
async def show_available_assignments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message += "4. Пропущенные задания можно выполнить в любой момент\n\n"
    message += "⏰ Новые задания открываются в 06:00 по вашему времени"
    ...
    row = []
    for assignment in available_assignments[:12]:  # Ограничим 12 кнопками
        # Определяем статус
...
async def show_course_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for arc in detailed_arcs:
        ...
        if len(row) == 1:  # По одной кнопке в строке (чтобы было читаемо)
            keyboard.append(row)
            row = []
    ...
    if current_arc:
        ...
        message += f"• Включает в себя доступ к трем первым заданиям активной части тренинга, обратную связь от психологаа выполненные задания и доступ к соотбществу\n\n"
    
    # Показываем будущие части
    future_arcs = [a for a in detailed_arcs if a['status'] == 'будущая']
    if future_arcs:
        message += f"**⏳ БУДУЩИЕ ЧАСТИ:**\n"
        for arc in future_arcs[:7]:  # Показываем только ближайшие 3
...
async def go_to_community(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    GROUP_LINK = "https://t.me/+P6-V_2perngxNzVi"
    ...
    for user_id, username, timezone_offset, city, phone in users:
        try:
            ...
            for arc_id, arc_title, arc_start in user_arcs:
                ...
                if user_hour == 7 and user_minute == 0:
...
async def show_assignment_from_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Убираем статус в скобках если есть
    if " (" in assignment_text:
        assignment_title = assignment_text.split(" (")[0].strip()
    else:
        assignment_title = assignment_text

    print(f"🔍 Извлекли название: '{assignment_title}' из '{assignment_text}'")
    
    # Получаем доступные задания из контекста
    ...
    day_id = None  # Найдем ниже
    
    # Получаем day_id для задания
    conn = sqlite3.connect('mentor_bot.db')
    cursor = conn.cursor()
    ...
    # Показываем задание (используем существующую функцию show_assignment)
    ...
    # 2. Отправляем текст задания
    if content_text:
        # Используем send_long_message или обрезаем
        short_content = content_text[:4000] + "..." if len(content_text) > 4000 else content_text
        await update.message.reply_text(f"📋 **Задание:**\n\n{short_content}", parse_mode='Markdown')

    # 3. Отправляем выбор ответа
...
async def show_feedback_list(update: Update, context: ContextTypes.DEFAULT_TYPE, viewed=0):
    ...
    for i, (assignment_title, day_title, day_num, comment, answer, assignment_id) in enumerate(assignments[:15]):  # Ограничим 15
        # Формируем текст кнопки
        clean_title = assignment_title
...
async def show_feedback_assignment_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    full_message += f"**День:** {day_title} ({day_num})\n\n"
    
    if content_text:
        # Ограничиваем длину текста задания
        content_preview = content_text[:4000] + ("..." if len(content_text) > 4000 else "")
        full_message += f"**Задание:**\n{content_preview}\n\n"
    
    if answer_text:
        # Ограничиваем длину ответа
        answer_preview = answer_text[:4000] + ("..." if len(answer_text) > 4000 else "")
...
async def buy_arc_from_catalog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if already_has:
            ...
            keyboard.append(["📚 Перейти в 'Мои задания'"])
        else:
