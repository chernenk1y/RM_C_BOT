...
# === ЮКАССА КОНФИГ ===
YOOKASSA_SHOP_ID = "1237477"
YOOKASSA_SECRET_KEY = "test_k9Sygc7OHD-r1Cqf8332lwK8MwjX9d1HbQ0uvcFd4Bs"
YOOKASSA_RETURN_URL = "https://t.me/PersonalityGrowth_bot"
...
def is_assignment_available(user_id, assignment_id):
    ...
    user_time = get_user_local_time(user_id)
    return user_time.hour < 22  # Доступно до 22:00
...
def init_db():
    ...
    conn.commit()
    conn.close()
    print("✅ База данных инициализирована (обновленная версия)")
...
def save_payment(user_id, arc_id, amount, yookassa_id, status='pending'):
    """Сохраняет платеж в БД - ОБНОВЛЕННАЯ ВЕРСИЯ"""
    import logging
...
def create_yookassa_payment(user_id, arc_id, amount, trial=False, description=""):
    """ВРЕМЕННО: Используем работающий код из тестовой команды"""
    import logging
    logger = logging.getLogger(__name__)
    
    logger.info(f"Используем исправленную версию платежа")
    
    # Копируем РАБОТАЮЩИЙ код из test_yookassa_keys
    import requests
    import base64
    import uuid
